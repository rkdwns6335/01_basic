...
public class Salary {
	public static void main(String[] args) {
		...
		int nomalPay = sc.nextInt();
		System.out.print("수당 입력 : ");
		int extraPay = sc.nextInt();
		
		int sum = (nomalPay + extraPay); // 합계
		double tax = (sum >= 5000000) ? sum * 0.03 : (sum >= 3000000) ? sum * 0.02 : sum * 0.01;//세율
		// [첫 조건] ? [결과값] : [두번째 조건] ? : [결과값] : [나머지] : [결과값]
		double salary = (sum - tax); //월급
		
		
		System.out.println("*** "+name+" " +rank+" 월급 ***");
		System.out.println("기본급 : " + df.format(nomalPay) + "원");
